...
read -r -a budgets <<< "${BUDGETS:-500 1800}"
read -r -a seeds <<< "${SEEDS:-0 1 2}"
episodes="${EPISODES:-500}"
oracle_episodes="${ORACLE_EPISODES:-50}"
log_dir="${LOG_DIR:-results/incremental/effect32_film_scaling/run_logs}"
mkdir -p "$log_dir"
  ...
  for seed in "${seeds[@]}"; do
    if [[ "$stage" == "train" || "$stage" == "all" ]]; then
      run_logged "train_n${budget}_seed${seed}" \
        uv run hcl-poc incremental effect32-scaling-train \
        --config "$config" \
    ...
    if [[ "$stage" == "eval" || "$stage" == "all" ]]; then
      run_logged "eval_n${budget}_seed${seed}_d${episodes}_o${oracle_episodes}" \
        uv run hcl-poc incremental effect32-scaling-eval \
        --config "$config" \
        --n-trajectories "$budget" \
        --seed "$seed" \
        --episodes "$episodes" \
