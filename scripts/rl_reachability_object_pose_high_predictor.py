...
def _evaluate(
    ...
    xy_error = np.linalg.norm(prediction[:, :2] - y[:, :2], axis=-1)
    angle_error = np.arctan2(
        prediction[:, 2] * y[:, 3] - prediction[:, 3] * y[:, 2],
        prediction[:, 2] * y[:, 2] + prediction[:, 3] * y[:, 3],
    )
    full_l2 = np.linalg.norm(prediction - y, axis=-1)
    return {
        "validation_object_pose_l2": float(np.mean(full_l2)),
        "validation_xy_l2_m": float(np.mean(xy_error)),
        "validation_yaw_abs_rad": float(np.mean(np.abs(angle_error))),
    }
...
def train(args: argparse.Namespace) -> Path:
    ...
    train_x, train_y = _samples(train_episodes, action_norm, horizon, control_freq)
    val_x, val_y = _samples(val_episodes, action_norm, horizon, control_freq)
    ...
    best_l2 = float("inf")
    history = []
    for epoch in trange(1, int(args.epochs) + 1, desc="train object-pose high predictor"):
        model.train()
        total = 0.0
        for x, y in loader:
        ...
        if metrics["validation_object_pose_l2"] < best_l2:
            best_l2 = metrics["validation_object_pose_l2"]
            best_state = copy.deepcopy(model.state_dict())
    if best_state is None:
        raise RuntimeError("Object-pose high predictor produced no checkpoint")

    model.load_state_dict(best_state)
    model.eval()
    metrics = _evaluate(model, val_x, val_y, input_norm, target_norm, device)
    current_object_pose = _pre_rl_phase_b_goal(
        val_x[:, :31],
        val_x[:, :31],
        horizon,
        control_freq,
    ...
    persistence_metrics = {
        "persistence_object_pose_l2": float(
            np.mean(np.linalg.norm(current_object_pose - val_y, axis=-1))
        ),
        "persistence_xy_l2_m": float(
            np.mean(np.linalg.norm(current_object_pose[:, :2] - val_y[:, :2], axis=-1))
        ),
    }
    payload = {
        "method": "rl_reachability_object_pose_high_predictor",
        "goal_type": GOAL_TYPE,
        "horizon_steps": horizon,
        "model": best_state,
